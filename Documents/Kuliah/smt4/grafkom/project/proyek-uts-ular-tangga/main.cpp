...
void persegi(float x, float y, float size, float red, float green, float blue) {
    ...
    glVertex2f(x + size, y);              // Titik kanan bawah
    glVertex2f(x + size, y + size);       // Titik kanan atas
...
// Koordinat pemain
float triangleX = -0.88f;
float triangleY = -0.96f;
float squareX = -0.88f;
float squareY = -0.91f;

float playerCoordinates[ROWS][COLS][2];
int tanggaMulai[] = {13, 36, 71}; // awal tangga
int tanggaAkhir[] = {44, 62, 75}; // akhir tangga
int ularMulai[] = {35, 69, 76}; // awal ular
int ularAkhir[] = {9, 31, 12}; //akhir ular
...
// mengkonversi nomor sel ke koordinat baris dan kolom
void posisicell(int cellNum, int* row, int* col) {
    // Sel dimulai dari 1, jadi kurangi 1
    cellNum = cellNum - 1;
    *row = cellNum / COLS;

    // Arah baris bergantian
    if (*row % 2 == 0) {
...
void koordinatcell(int cellNum, float* x, float* y) {
    if (cellNum < 1 || cellNum > ROWS * COLS) {
        // Handle invalid cell numbers
        *x = -0.9f;
        *y = -0.9f;
        return;
    }
    ...
    int row, col;
    posisicell(cellNum, &row, &col);

    float cellWidth = 2.0f / COLS;
    float cellHeight = 2.0f / ROWS;

    // Hitung posisi koordinat (sesuaikan dengan drawBoard)
    *x = -1.0f + col * cellWidth + cellWidth * 0.25f; // offset untuk tidak tepat di tepi
    *y = -1.0f + row * cellHeight + cellHeight * 0.25f;
}

// Fungsi untuk memeriksa apakah pemain berada di awal tangga atau ular
void cekultang(int playerIndex) {
...
void input(unsigned char key, int x, int y) {
    if (key == ' ' && !gameOver) {
        ...
        if (playerPositions[currentPlayer] >= ROWS * COLS) {
            ...
            cekultang(currentPlayer);
        }

        // Update posisi visual pemain
        if (currentPlayer == 0) {
            // Update posisi segitiga
            koordinatcell(playerPositions[0], &triangleX, &triangleY);
        } else {
            // Update posisi persegi
            koordinatcell(playerPositions[1], &squareX, &squareY);
        }
...
void display() {
    ...
    //pemain
    segitiga(triangleX, triangleY, 0.05f, 0.0f, 1.0f, 0.0f); // 1
    persegi(squareX, squareY, 0.05f, 1.0f, 1.0f, 0.0f); // 2

    // info permainan
    char infoText[100];
    sprintf(infoText, "pemain %d' Tekan SPASI untuk melempar dadu", currentPlayer + 1);
    glColor3f(1.0f, 1.0f, 1.0f);
    drawText(-0.9f, -1.05f, infoText);

    if (diceResult > 0) {
        sprintf(infoText, "Last roll: %d", diceResult);
        drawText(0.0f, -1.05f, infoText);
    }

    if (gameOver) {
        sprintf(infoText, "pemain %d menang! tekan R untuk memulai ulang", 1 - currentPlayer + 1);
        drawText(0.3f, -1.05f, infoText);
    }
